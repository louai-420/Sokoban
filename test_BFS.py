...
def bfs(curr_player, curr_boxes):
	...
	frontier = Queue()
	explored = set()
	frontier.put((curr_player, curr_boxes, 0, []))

	node_generated += 1
	explored.add((curr_player, curr_boxes))
	startTime = time.time()
	while True:
		(now_player, now_boxes, step, actions) = frontier.get()
		...
		for m in moves:
			...
			if (new_player, new_boxes) not in explored and res == True:
				...
				frontier.put((new_player, new_boxes, step+1, actions + [m]))
			node_generated += 1
...
if __name__ == '__main__':
	...
	for j in range(i, 80):
		...
		(node_created, step, times, memo) = bfs(player, boxes)


		f = open("BFS.csv", 'a+')
		f.write("{},{},BFS,{},{},{:0.6f},{:0.6f}\n".format(map_list[int(j/40)], j%40+1, node_created, step, times, memo))
